...
public class SWTGraphicUtil
{
	 ...
	 * 
	 * @param source
	 *        source image to be scaled and reflected
	 * @param newWidth
	...
	/**
	 * Apply a very basic pseudo-HTML formating to a text stored in a StyledText widget. Supported tags are <b>, <i>, <u> , <COLOR>, <backgroundcolor>, <size>
	 * and <BbrR/>
	 * 
	 * @param styledText
	 ...
	 *        The ImageData to be average blurred. Transparency information will be ignored.
	 * @param radius
	...
	public static ImageData blur(final ImageData originalImageData, int radius)
	{
		...
		while (targetRow < newImageData.height) {
			...
			for (int col = 0; col < newImageData.width; col++) {
				rowRGBAverages[col] = newImageData.palette.getPixel(new RGB(rowRGBSums[col].red / numRows, rowRGBSums[col].green / numRows,
